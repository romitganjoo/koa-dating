	...
	app.post('/api/resetpassword', (request, response) => {
		...
		const createResetCode = async (rows) => {
			...
			try {
				...
				const mailInfo = { username: rows[0]['username'], email: rows[0]['email'], code: emailCode, firstname: rows[0]['firstname'] }
		...
		const sendResetMail = async (mailInfo) => {
			var mailOptions = {
				...
				to: mailInfo.email,
				subject: 'Zuv password reset',
				html: `<h3>Hello, ${mailInfo.firstname}!</h3><p>It seems like you have forgotten your password!</p>
